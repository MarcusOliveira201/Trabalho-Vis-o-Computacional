...
def merge_PPM(entered_input, filters=48):
    out_1 = convolution_block(entered_input, filters, 1,1)
    out_3 = convolution_block(entered_input, filters, 3,2)
    out_7 = convolution_block(entered_input, filters, 7,3)
    out_11 = convolution_block(entered_input, filters, 11,4)
...
def UNET_PPM_ASPP(image_size, num_classes,activation):
...
def mytest():
    ...
    model = UNET_PPM_ASPP(image_size=IMAGE_SIZE, num_classes=NUM_CLASSES, activation="softmax")
    model.summary()
