...
def merge_PPM(entered_input, filters=48):
    ...
    out_1 = DeformableConvLayer(filters=48, kernel_size=1, strides=1, padding='same', dilation_rate=1, num_deformable_group=1)(entered_input)
    out_3 = DeformableConvLayer(filters=48, kernel_size=3, strides=1, padding='same', dilation_rate=1, num_deformable_group=1)(entered_input)
    out_5 = DeformableConvLayer(filters=48, kernel_size=5, strides=1, padding='same', dilation_rate=1, num_deformable_group=1)(entered_input)
    out_7 = DeformableConvLayer(filters=filters, kernel_size=7, strides=1, padding='same', dilation_rate=1, num_deformable_group=1)(entered_input)
    
    # out_1 = convolution_block(entered_input, filters, 1)
    # out_3 = convolution_block(entered_input, filters, 3)
    # out_7 = convolution_block(entered_input, filters, 5)
    # out_11 = convolution_block(entered_input, filters, 7)
    ...
    merged = layers.concatenate([out_1, out_3, out_5, out_7], axis=-1)
...
def UNET_PPM(image_size, num_classes,activation):
...
def UNet_Enconder(input):
    ...
    skip1, encoder_1 = DeformableConvLayer(filters=6, kernel_size=3, strides=1, padding='valid', dilation_rate=1, num_deformable_group=1)(input1)
    ...
    conv_block = bottleneck_block(encoder_1,  n_filtro*16)
    
    return [skip1],conv_block
...
def mytest():
    ...
    model = UNET_PPM(image_size=IMAGE_SIZE, num_classes=NUM_CLASSES, activation="softmax")
    model.summary()
